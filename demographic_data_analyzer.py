...
def calculate_demographic_data(print_data=True):
    # Lendo o arquivo CSV e ignorando a linha inicial incorreta
    df = pd.read_csv('adult.data.csv', skiprows=1, header=None, names=[
        'age', 'workclass', 'fnlwgt', 'education', 'education-num', 'marital-status',
        'occupation', 'relationship', 'race', 'sex', 'capital-gain', 'capital-loss',
        'hours-per-week', 'native-country', 'salary'
    ])

    # Converter colunas que precisam ser numéricas
    ...
    country_salary_df = df[df['salary'] == '>50K'].groupby('native-country')['salary'].count() / df.groupby('native-country')['salary'].count()
